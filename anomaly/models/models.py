        ...
        if segSize is None:
            ...
            for i in range(n):
                ...
                for instance in instances:
                    ...
                    if instance == -1:  # Ignore background
                        continue
            ...
            # loss = torch.exp(self.alpha * VAR_loss/n) + torch.exp(self.beta * Inter_loss/n)
            # print(CE_loss / n, torch.exp(self.alpha * VAR_loss/n), torch.exp(self.beta * Inter_loss/n))
            # loss = CE_loss / n + torch.exp(self.alpha * VAR_loss / n) + torch.exp(self.beta * Inter_loss / n)
            loss = 1 * CE_loss / n + self.alpha * VAR_loss / n
    ...
    def build_decoder(arch='ppm_deepsup',
        ...
        if arch == 'c1_deepsup':
            ...
            net_decoder = PPMDeepsup(
                ...
                fc_dim=fc_dim,
                use_softmax=use_softmax)
        elif arch == 'ppm_deepsup_embedding':
            net_decoder = PPMDeepsup_embedding(
                num_class=num_class,
                fc_dim=fc_dim,
                use_softmax=use_softmax)
...
class PPMDeepsup(nn.Module):
    ...
    def forward(self, conv_out, segSize=None, output_ft = True):
        conv5 = conv_out[-1]
...
class PPMDeepsup_embedding(nn.Module):
    def __init__(self, num_class=150, fc_dim=4096,
                 use_softmax=False, pool_scales=(1, 2, 3, 6)):
        super(PPMDeepsup_embedding, self).__init__()
        self.use_softmax = use_softmax
        ...
        self.centers = torch.zeros(13, 13)
        magnitude = 3
        for i in range(13):
    ...
    def forward(self, conv_out, segSize=None, output_ft = True):
        ...
        if self.use_softmax:  # is True during inference
            ...
            if output_ft:
                # ft = nn.functional.interpolate(
                #    ft, size=segSize, mode='bilinear', align_corners=False)
                features_out = nn.functional.interpolate(
                    features_out, size=segSize, mode='bilinear', align_corners=False)
                return x, features_out
                return x, ft
            else:
                return x
