...
def validate(opts, model, loader, device, metrics, ret_samples_ids=None):
    ...
    #
    # data_list = [[] for _ in range(opts.num_classes + 1)]
    #
    # with torch.no_grad():
    ...
    #         preds = outputs.detach().max(dim=1)[1].cpu().numpy()             # 1 * 1 * h * w
    #         soft_outs = F.softmax(outputs, dim=1)                            # 1 * dim * h * w
    #         dis_sum_map = -np.sum(outputs.squeeze().cpu().numpy(), axis=0)   # 1 * h * w
    #         feat_np = dis_sum_map.ravel()
    #         prds_np = preds.ravel()
    #         true_np = labels.cpu().numpy().ravel()
    #         for cl in range(opts.num_classes):
    #             if np.any((true_np == cl) & (prds_np == cl)):
    #                 cl_feat_flat = feat_np[(true_np == cl) & (prds_np == cl)]
    #                 cl_feat_flat = np.random.choice(cl_feat_flat, int(len(cl_feat_flat) / 500))
    #                 data_list[cl] += cl_feat_flat.tolist()
    #         cl_feat_flat = np.random.choice(feat_np[true_np == 255], int(len(feat_np[true_np == 255]) / 500))
    #         cl_feat_flat = cl_feat_flat[cl_feat_flat < 1000]
    #         data_list[-1] += cl_feat_flat.tolist()
    #
    #
    ...
    # for cl in range(opts.num_classes):
    #     plt.figure()
    #     plt.hist(data_list[cl], bins=100)
    #     plt.show()
    #     statistical_model = GaussianMixture(n_components=2, random_state=0)
    #     statistical_model.fit(np.array(data_list[cl]).reshape(-1,1))
    #     model_list.append(statistical_model)
    #     # print(statistical_model.means_)
    #     # print(statistical_model.covariances_)
    #     ecdf = ECDF(data_list[cl])
    #     ecdf_list.append(ecdf)
    #     min_cov = np.min(statistical_model.covariances_)
    #     min_mean = statistical_model.means_[np.argmin(statistical_model.covariances_)]
    #     thre = min_mean - 1 * np.sqrt(min_cov)
    #     thre_list.append(thre)
    #     print(thre)
    #
    # plt.figure()
    # plt.hist(data_list[-1], bins=100)
    # plt.show()
    #
...
def main():
    ...
    utils.mkdir('checkpoints_bs8_embedding_161718')
    # Restore
    best_score = 0.0
    cur_itrs = 0
    cur_epochs = 0
    ...
    while True: #cur_itrs < opts.total_itrs:
        ...
        for (images, labels, labels_true) in train_loader:
            ...
            if (cur_itrs) % opts.val_interval == 0:
                save_ckpt('checkpoints_bs8_embedding_161718/latest_%s_%s_os%d.pth' %
                ...
                if val_score['Mean IoU'] > best_score:  # save best model
                    ...
                    save_ckpt('checkpoints_bs8_embedding_161718/best_%s_%s_os%d.pth' %
                              (opts.model, opts.dataset,opts.output_stride))
