...
def get_dataset(opts):
    ...
    if opts.dataset == 'cityscapes':
        ...
        train_dst = Cityscapes(root=opts.data_root,
                               split='test_car', transform=train_transform)
...
def main():
    ...
    utils.mkdir('checkpoints_bs8_embedding_1514_ft_1')
    # Restore
    best_score = 0.0
    cur_itrs = 0
    cur_epochs = 0
    ...
    while True: #cur_itrs < opts.total_itrs:
        ...
        for (images, labels, labels_true) in train_loader:
            ...
            for i in range(opts.novel_cls - 1):
                labels_base_tmp = outputs[i+1].detach().max(dim=1)[1]
                labels_base[labels_base_tmp == (16+i)] = (16+i)
            #
            # labels[labels == 0] = 16 + opts.novel_cls - 1
            # labels[labels == 255] = labels_base[labels == 255]

            labels_base[labels_base >= 13] += 3
            labels_copy = labels.clone()
            labels_copy[labels_copy != 13] = labels_base[labels_copy !=13]
            plt.figure()
            plt.imshow(colorEncode(labels_copy[0].cpu().numpy(), colors))
            plt.xticks([])
            plt.yticks([])
            plt.axis('off')
            plt.savefig('labels/labels_false' + str(index) + '.jpg', bbox_inches='tight', dpi=600, pad_inches=0)
            plt.show()
            plt.close()
            ...
            #
            labels[labels >= 14 ] += 2
            labels[labels == 257] = 19
            plt.figure()
            plt.imshow(colorEncode(labels[0].cpu().numpy(),colors))
            plt.xticks([])
            plt.yticks([])
            plt.axis('off')
            plt.savefig('labels/labels_true'+str(index)+'.jpg', bbox_inches='tight', dpi=600, pad_inches=0)
            plt.show()
            plt.close()
            ...
            if (cur_itrs) % opts.val_interval == 0:
                save_ckpt('checkpoints_bs8_embedding_1514_ft_1/latest_%s_%s_os%d.pth' %
                ...
                if val_score['Mean IoU'] > best_score:  # save best model
                    ...
                    save_ckpt('checkpoints_bs8_embedding_1514_ft_1/best_%s_%s_os%d.pth' %
                              (opts.model, opts.dataset,opts.output_stride))
